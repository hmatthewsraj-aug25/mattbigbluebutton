...
func (g SVGGenerator) Generate(msg pipeline.Message[*document.Presentation]) (pipeline.Message[*document.Presentation], error) {
	...
	for _, page := range msg.Payload.Pages {
		...
		for {
			var fontErr error
			if rasterize, fontErr = detector.HasFontType3(page.FilePath, page.Num); fontErr != nil {
				if attempt > g.cfg.Generation.SVG.PDF.Font.MaxAttempts {
					slog.Error("failed to generate SVG", "file", pres.FilePath, "page", page.Num,
		...
		if !rasterize {
			ctx, cancel := context.WithTimeout(context.Background(), time.Duration(timeout)*time.Second)
			defer cancel()
			cmd := document.NewGenerationProcess(document.GenerationProcessPDFToCairo).Resolution(g.cfg.Generation.SVG.Resolution).
				Format(document.GenerationProcessFormatSVG).Pages(page.Num, page.Num).InputOutput(pres.FilePath, svg).
				Execute(g.exec, timeout, ctx)
			output, genErr := cmd.CombinedOutput()
			if genErr != nil {
				slog.Error("Failed to generate SVG", "file", pres.FilePath, "page", page.Num, "error", genErr, "output", output)
			}
		}
		...
		numImgTags, _ := document.CountSVGImageTags(svg)
		numTags, _ := document.CountSVGTags(svg)
		...
		if fileEmpty || maxImages || maxTags || rasterize {
			...
			defer cancel()
			cmd := document.NewGenerationProcess(document.GenerationProcessPDFToCairo).Resolution(g.cfg.Generation.SVG.Resolution).
				Format(document.GenerationProcessFormatPNG).Rasterize(g.cfg.Generation.SVG.Rasterize.Width).Pages(page.Num, page.Num).
				InputOutput(pres.FilePath, png).Execute(g.exec, timeout, ctx)
			output, genErr := cmd.CombinedOutput()
			if genErr != nil {
				slog.Error("Failed to generate PNG", "file", pres.FilePath, "page", page.Num, "error", genErr, "output", output)
			}
