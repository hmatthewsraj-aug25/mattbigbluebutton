...
// SendReceive sets the processor for a [Step] to the provided [SenderReceiver].
func (s *Step[T, U]) SendReceive(sr SenderReceiver[T, U]) *Step[T, U] {
	if s.processor != nil {
		panic("cannot call SendReceive on a Step that already has a processor function")
	}
	s.processor = sr.Send
	return s
}
